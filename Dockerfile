...
COPY --from=builder /crawler.debug.app /
COPY --from=builder /dlv /

EXPOSE 2345
ENTRYPOINT ["/dlv", "--headless=true", "--listen=:2345", "--api-version=2", "--accept-multiclient", "--log", "exec", "/crawler.debug.app"]

########### PROD IMAGE ###################
FROM gcr.io/distroless/static as prod

# Import the binary
COPY --from=builder /crawler.app /

# Google's distroless uses nobody:nobody
ENTRYPOINT ["/crawler.app"]